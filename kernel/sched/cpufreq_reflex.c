 ...
 * Original: Copyright (C) 2026 Masahito Suzuki <firelzrd@gmail.com>
 *
 * Ported to Linux 4.19 / MediaTek MT6768 (SSPM/NONLINEAR_FREQ_CTL) tree
 * by Rize, 2026. Structurally mirrors this tree's kernel/sched/cpufreq_schedutil.c
 * (including the SSPM DVFS-offload path) with the addition of firelzrd's
 * idle-time based "hispeed" floor blended with PELT utilization.
 *
 ...
 * get_cpu_idle_time() idle-time counters.  This is blended with PELT util:
 *
...
#define CPUFREQ_REFLEX_AUTHOR   "Masahito Suzuki (ported by Rize)"
#define CPUFREQ_REFLEX_VERSION  "0.3.2-mtk"
...
/* Reuse the notifier function pointer exported/defined by cpufreq_schedutil.c */
...
#else
static void rfx_fast_switch(struct rfx_policy *rfx_pol, u64 time,
			     unsigned int next_freq)
{
	struct cpufreq_policy *policy = rfx_pol->policy;
	int cpu;

	if (!rfx_update_next_freq(rfx_pol, time, next_freq))
		return;
...
/*
 * Compute the next frequency for a given util/max pair.
 * Mirrors this tree's get_next_freq()/cpufreq_schedutil_plus.c logic
 * (including the NONLINEAR_FREQ_CTL / SSPM paths) so reflex plugs into
 * the exact same DVFS pipeline schedutil already uses on this device.
 */
				       ...
				       unsigned long util, unsigned long max)
{
	...
	freq = mtk_map_util_freq(policy->cpu, util);
#else
...
/*
 * Get CPU utilization using the same schedutil_cpu_util() helper schedutil
 * uses (declared in kernel/sched/sched.h), so reflex sees identical PELT,
 * uclamp and DL-bandwidth accounting.
 */
static unsigned long rfx_get_util(struct rfx_cpu *rfx_c)
{
	...
	rfx_c->max = max;
	rfx_c->bw_dl = cpu_bw_dl(rq);

	return schedutil_cpu_util(rfx_c->cpu, util_cfs, max,
				   FREQUENCY_UTIL, NULL);
}
...
/*
 * Update busy% using kernel idle-time accounting (kcpustat), giving the raw
 * CPU busy ratio without PELT smoothing.
 */
 ...
 * continuous exponential decay. See header comment for the full formula.
 */
				       ...
				       unsigned long util, unsigned long max)
{
	...
	boost = (rfx_c->iowait_boost * max) >> SCHED_CAPACITY_SHIFT;
	return max_t(unsigned long, boost, util);
}
...
/*
 * Make rfx_should_update_freq() ignore the rate limit when DL
 * has increased the utilization.
 */
static inline void rfx_ignore_dl_rate_limit(struct rfx_cpu *rfx_c,
					     struct rfx_policy *rfx_pol)
{
	if (cpu_bw_dl(cpu_rq(rfx_c->cpu)) > rfx_c->bw_dl)
		WRITE_ONCE(rfx_pol->limits_changed, true);
}

static inline void __rfx_cpufreq_notifier_fp(int cid, unsigned int next_f)
{
			       ...
			       unsigned int flags)
{
	...
	rfx_ignore_dl_rate_limit(rfx_c, rfx_pol);
	...
	busy = !rfx_pol->need_freq_update && rfx_cpu_is_busy(rfx_c);

	util = rfx_get_util(rfx_c);
	max = rfx_c->max;
	util = rfx_iowait_apply(rfx_c, time, util, max);
	...
	next_f = rfx_get_next_freq(rfx_pol, util, max);
	...
	if (rfx_update_next_freq(rfx_pol, time, next_f)) {
		if (policy->fast_switch_enabled)
			rfx_fast_switch(rfx_pol, time, next_f);
		else
			rfx_deferred_update(rfx_pol);
	}
#endif

	__rfx_cpufreq_notifier_fp(cid, next_f);
	raw_spin_unlock(&rfx_pol->update_lock);
}
...
static unsigned int rfx_next_freq_shared(struct rfx_cpu *rfx_c, u64 time)
{
	...
	for_each_cpu(j, policy->cpus) {
		...
		j_util = rfx_get_util(j_rfx_c);
		j_util = rfx_iowait_apply(j_rfx_c, time, j_util, max_cap);
...
rfx_update_shared(struct update_util_data *hook, u64 time, unsigned int flags)
{
	...
	cid = arch_cpu_cluster_id(rfx_pol->policy->cpu);
...
/* Macro to define show/store for plain unsigned int tunables */
