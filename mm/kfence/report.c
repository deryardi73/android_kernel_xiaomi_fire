...
/*
 * KFENCE reporting. Backported to 4.19 for inferno kernel (fire/MT6768).
 *
 * Copyright (C) 2020, Google LLC.
 *
 * Backport notes:
 *  - `no_hash_pointers` extern removed (doesn't exist on this tree); corrupted
 *    canary bytes are always masked ("!") rather than conditionally shown in
 *    hex, matching the safe/non-debug upstream default.
 *  - trace_error_report_end()/error_report tracepoint removed (added far
 *    later upstream, no equivalent header here). Purely a trace-event no-op
 *    removal; the pr_err() report itself is unaffected.
 */
