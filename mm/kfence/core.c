 ...
 * Backported to 4.19 for inferno kernel (fire/MT6768).
 *
 * Copyright (C) 2020, Google LLC.
 *
 * Backport notes:
 *  - KCSAN scoped-access calls dropped (kcsan-checks.h not present on this
 *    tree; no KCSAN support backported). Functionally a no-op removal, does
 *    not affect KFENCE's own OOB/UAF detection via guard pages.
 *  - data_race() dropped (KCSAN-only annotation macro); replaced with plain
 *    reads, semantics unchanged.
 *  - No stack_depot use (see mm/kfence/kfence.h backport note).
 */
...
static void kfence_guarded_free(void *addr, struct kfence_metadata *meta, bool zombie)
{
	...
	/* No KCSAN scoped-access on this tree; racy UAF still caught via guard page fault. */
...
bool kfence_handle_page_fault(unsigned long addr, bool is_write, struct pt_regs *regs)
{
	...
	if (page_index % 2) {
		...
		meta = addr_to_metadata(addr + PAGE_SIZE);
		if (meta && READ_ONCE(meta->state) == KFENCE_OBJECT_ALLOCATED) {
			if (!to_report || distance > (int)(meta->addr - addr))
				to_report = meta;
		}
