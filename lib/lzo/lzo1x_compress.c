 ...
 *  Full-file port (not hand-patched diff): this is dense bitstream-format
 *  code where a single wrong byte offset silently corrupts every
 *  zram-compressed page, so it's taken verbatim rather than patched.
 *  Dependencies checked present on this tree: get_unaligned/put_unaligned,
 *  BUILD_BUG_ON, CONFIG_HAVE_EFFICIENT_UNALIGNED_ACCESS (arm64 selects it).
 */
